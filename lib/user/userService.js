...
function register(req, res) {
	...
	emoji_schema
		.findOne({ $or: [{ emailId: req.body.emailId }, { contactNumber: req.body.contactNumber }] })
		.then(async result => {
			if (result) {
				return res.json({
					code: userConst.CODES.BADREQUEST,
					message: userConst.MESSAGES.emailAlreadyRegistered,
				});
			} else {
				...
				User.save(async data => {
					...
					await util.sendMail(req.body.emailId, userConst.MAILOBJ.regiterMailSubject, html).then(body => {
						if (body) {
							return res.json({
								code: userConst.CODES.SUCCESS,
								message: userConst.MESSAGES.sucessfullyRegister,
							});
						} else {
							return res.json({
								code: userConst.CODES.INTRNLSRVRERR,
		...
		.catch(err => {
			res.json({ code: userConst.CODES.INTRNLSRVRERR, message: userConst.MESSAGES.internalServerError });
		});
}
	...
	emoji_schema
		...
		.then(result => {
			if (req.body.pin == result.pin) {
				emoji_schema
					.findOneAndUpdate({ _id: req.params.id }, { $set: { isVerify: true } }, { new: true })
					.then(data => {
						return res.json({
							code: userConst.CODES.SUCCESS,
							message: userConst.MESSAGES.pinVerifieldSucess,
						});
					});
			} else {
	...
	emoji_schema.findOne({ _id: req.params.id }).then(async data => {
		if (data) {
			if (data.isVerify == true) {
				...
				await util
					.sendMail(data.emailId, userConst.MAILOBJ.regiterMailSubject, html)
					.then(result => {
						if (result) {
							emoji_schema.findOneAndUpdate(
								{ _id: req.params.id },
								{ $set: { pin: newPin } },
								{ new: true },
								(err, body) => {
									return res.json({
										code: userConst.CODES.SUCCESS,
										message: userConst.MESSAGES.pinSentsucess,
									});
								}
							);
						} else {
...
function login(req, res) {
	emoji_schema
		.findOne({ emailId: req.body.emailId }, async (err, data) => {
			if (err) {
				return res.json({
					code: userConst.CODES.INTRNLSRVRERR,
					message: userConst.MESSAGES.internalServerError,
				...
				if (data.role == 'ADMIN') {
					...
					if (match) {
						...
						return res.json({
							code: userConst.CODES.BADREQUEST,
							message: userConst.MESSAGES.invalidPassword,
						});
					}
...
function forgotPassword(req, res) {
	emoji_schema.findOne({ emailId: req.body.emailId }, async (err, data) => {
		if (err) {
			...
			util.sendMail(data.emailId, userConst.MAILOBJ.forgotPasswordMailSubject, html)
				.then(result => {
					if (result) {
						...
						emoji_schema.findOneAndUpdate(
							...
							(err, result1) => {
								return res.json({
									code: userConst.CODES.SUCCESS,
									message: userConst.MESSAGES.forgetPasswordMail,
								});
							}
						);
					}
		...
		.then(result => {
			{
				var match = bcrypt.compare(req.body.oldPassword, result.password).then(result1 => {
					if (result1 == true) {
						emoji_schema
							.findOneAndUpdate({ _id: result._id }, { $set: { password: newpass } })
							.then(result2 => {
								return res.json({
									code: userConst.CODES.SUCCESS,
									message: userConst.MESSAGES.passwordUpdatedSucess,
								});
							});
					} else {
						return res.json({
