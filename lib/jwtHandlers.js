...
var genUsrToken = function(user) {
	...
	return jwt
		.signAsync(user, process.env.user_secret, options)
...
var genAdminToken = function(admin) {
	var options = { expiresIn: TOKEN_EXPIRATION_SEC };
	return jwt
		.signAsync(admin, process.env.admin_secret, options)
		.then(function(jwtToken) {
			return jwtToken;
		})
		.catch(function(err) {
...
var verifyUsrToken = function(jwtToken) {
	return jwt
		.verifyAsync(jwtToken, process.env.user_secret)
...
var verifyAdminToken = function(acsTokn) {
	return jwt
		.verifyAsync(acsTokn, process.env.admin_secret)
		.then(function(tokenPayload) {
			this.tokenPayload = tokenPayload;
			return tokenPayload;
		})
