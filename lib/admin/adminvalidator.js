		...
		jwtHndlr
			...
			.then(result => {
				if (result) {
					...
					return res.json({
						code: adminconst.CODE.INTERNALSERVER,
						message: adminconst.MESSAGE.INTRNLSRVRERR,
					});
				}
			})
			.catch(err => {
				return res.json({ code: adminconst.CODE.INTERNALSERVER, message: adminconst.MESSAGE.inValidToken });
			});
	}
}
